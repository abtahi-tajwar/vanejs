...
const renderBinds = (elements) => {
  elements.forEach((el) => {
    let key;
    key = el.dataset.bind;
    const value = templateEngineStates
      ? getValueByPath(templateEngineStates, key)
      : null;
    el.innerHTML = value;
  });
};
...
function renderRepeats(
  ...
  elements.forEach((el) => {
    ...
    if (!template)
      throw new Error(
        "Please define a <template></template> inside your data repeat"
      );
    ...
    for (let i = 0; i < arrVal.length; i++) {
      ...
      itemDOM
        ...
        .forEach((item) => item.removeAttribute("data-repeat"));
      ...
      rItems.forEach((item) => {
        ...
        const value = templateEngineStates
          ? getValueByPath(templateEngineStates, absolutepath)
          : null;
